...
const App: React.FC = () => {
  ...
  const currentPageInfo = PAGES[currentPath] || PAGES['/'];
  const [pageContent, setPageContent] = useState('');
  const [pageMetadata, setPageMetadata] = useState(currentPageInfo.metadata);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    const fetchContent = async () => {
      if (!currentPageInfo.file) return;

      setIsLoading(true);
      try {
        const response = await fetch(currentPageInfo.file);
        ...
        if (match) {
          ...
          // Parse basic metadata (title, description, status, etc.)
          const titleMatch = frontmatter.match(/title:\s*"(.*)"/);
          const descMatch = frontmatter.match(/description:\s*"(.*)"/);
          const statusMatch = frontmatter.match(/status:\s*"(.*)"/);
          const versionMatch = frontmatter.match(/version:\s*"(.*)"/);
          const updatedMatch = frontmatter.match(/lastUpdated:\s*"(.*)"/);
          // Contributors is an array, simpler parsing for now or assume default if complex

          setPageMetadata({
            ...currentPageInfo.metadata,
            title: titleMatch ? titleMatch[1] : currentPageInfo.metadata.title,
            description: descMatch ? descMatch[1] : currentPageInfo.metadata.description,
            status: (statusMatch ? statusMatch[1] : currentPageInfo.metadata.status) as any,
            version: versionMatch ? versionMatch[1] : currentPageInfo.metadata.version,
            lastUpdated: updatedMatch ? updatedMatch[1] : currentPageInfo.metadata.lastUpdated,
          });
    ...
    fetchContent();
  }, [currentPath, currentPageInfo]);

  const currentPage = { ...currentPageInfo, metadata: pageMetadata, content: pageContent };
          ...
          {isHome ? (
            ...
            <div className="max-w-4xl mx-auto px-6 py-12 lg:px-16 lg:py-20">
              ...
              <div className="flex items-center gap-4 mb-8">
                <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ring-1 ${currentPage.metadata.status === 'stable' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' :
