  ...
  <div className="relative">
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center relative z-10">
        <div>
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-100 text-emerald-800 rounded-full text-xs font-bold uppercase tracking-wider mb-6 shadow-sm">
            ...
            Phase de préfiguration en cours
          </div>
          <h1 className="text-4xl lg:text-6xl font-black tracking-tight text-slate-900 mb-6 leading-[1.1]">
            Vos données forestières.<br />
            <span className="text-emerald-600">Enfin interopérables.</span>
          </h1>
          <p className="text-lg lg:text-xl text-slate-600 leading-relaxed mb-8 max-w-xl">
            Rejoignez le consortium qui pose les bases du futur espace de données partagé de la filière forêt-bois en France.
          </p>
    ...
    <div className="py-24 px-6 lg:px-8 bg-white">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-3xl lg:text-4xl font-black text-slate-900 mb-4">Un constat partagé dans la filière</h2>
          <p className="text-lg text-slate-500 max-w-2xl mx-auto">La filière forêt-bois dispose de nombreuses initiatives autour des données. Mais elles ne se parlent pas.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-8">
          <div className="bg-slate-50 rounded-2xl p-8 border border-slate-100 hover:border-emerald-200 hover:shadow-lg transition-all">
            ...
            <h3 className="text-lg font-bold text-slate-900 mb-3">Initiatives fragmentées</h3>
            <p className="text-slate-600 text-sm leading-relaxed">Observatoires, plateformes, référentiels... De nombreux projets existent mais restent isolés.</p>
          </div>
          <div className="bg-slate-50 rounded-2xl p-8 border border-slate-100 hover:border-emerald-200 hover:shadow-lg transition-all">
            <div className="w-14 h-14 bg-red-100 text-red-600 rounded-2xl flex items-center justify-center text-2xl mb-6">🔗</div>
            <h3 className="text-lg font-bold text-slate-900 mb-3">Interopérabilité limitée</h3>
            <p className="text-slate-600 text-sm leading-relaxed">Les systèmes actuels ne communiquent pas entre eux, rendant la traçabilité complexe.</p>
          </div>
          <div className="bg-emerald-50 rounded-2xl p-8 border border-emerald-100 hover:shadow-lg transition-all">
            ...
            <h3 className="text-lg font-bold text-slate-900 mb-3">Un besoin commun</h3>
            <p className="text-slate-600 text-sm leading-relaxed">DataWood Consortium crée le socle partagé pour y remédier, sans imposer ni concurrencer.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="py-24 px-6 lg:px-8 bg-slate-50">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-3xl lg:text-4xl font-black text-slate-900 mb-4">Ce que fait DataWood</h2>
          <p className="text-lg text-slate-500">Une approche de structuration amont, collaborative et ouverte.</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { icon: '📋', title: 'Recense', desc: 'Inventorie les initiatives data existantes de la filière.' },
            { icon: '📚', title: 'Documente', desc: 'Analyse formats, référentiels et standards en place.' },
            { icon: '🔄', title: 'Connecte', desc: 'Met en cohérence les travaux pour créer des ponts.' },
            { icon: '📖', title: 'Partage', desc: 'Produit un état de l\'art accessible à tous.' },
            { icon: '🤝', title: 'Fédère', desc: 'Anime une communauté d\'acteurs engagés.' },
            { icon: '🚀', title: 'Prépare', desc: 'Pose les fondations du futur Data Space filière.' },
          ].map((item, i) => (
        ...
        <div className="mt-16 bg-white rounded-3xl p-8 lg:p-12 border border-slate-200 shadow-lg relative overflow-hidden">
          ...
          <div className="relative z-10 grid lg:grid-cols-2 gap-8 items-center">
            <div>
              <h3 className="text-2xl font-bold text-slate-900 mb-4">Ce que DataWood n'est pas</h3>
  ...
  <div className="bg-slate-50 min-h-screen pb-24">
    ...
    <div className="max-w-7xl mx-auto px-6 -mt-16 relative z-20">
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {/* Simulated Blog Posts Data - In real app, fetch this list */}
        {[
          {
            title: "Lancement officiel du DataWood Consortium",
            desc: "Une étape majeure pour la donnée forestière en France. Retour sur l'événement...",
            date: "15 Juin 2024",
            tag: "Annonce",
          ...
          },
          {
            ...
            desc: "Le Règlement Contre la Déforestation Importée change la donne. Voici ce que vous devez savoir.",
            date: "02 Juil 2024",
            tag: "Réglementation",
            img: "https://images.unsplash.com/photo-1621255557683-16239922e4d0?auto=format&fit=crop&q=80&w=800",
            slug: "/blog/rdue-decryptage"
          }
        ].map((post, i) => (
          <div key={i} onClick={() => navigateTo(post.slug)} className="group bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 cursor-pointer border border-slate-100 flex flex-col h-full">
            ...
            <div className="p-8 flex flex-col flex-grow">
              <div className="text-slate-400 text-sm font-medium mb-3 flex items-center gap-2">
                <span>{post.date}</span> • <span>5 min lecture</span>
              </div>
    ...
    const fetchContent = async () => {
      ...
      try {
        ...
        if (match) {
          ...
          setMetadata({
            title: titleMatch ? titleMatch[1] : 'Titre',
        ...
        setPageContent('# Article non trouvé');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="bg-white">
      ...
      <div className="max-w-3xl mx-auto px-6 py-16 lg:py-24 relative">
        <article className="prose prose-lg prose-slate prose-headings:font-bold prose-headings:text-slate-900 prose-a:text-emerald-600">
          {isLoading ? (
            <div className="flex justify-center py-10"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-emerald-600"></div></div>
          ) : (
            pageContent.split('\n').map((line, idx) => {
